...
Both halves of the web app's wind stack are live: the colored speed raster and the animated particle field on top of it.

`src/wind/` ports the web app's `WindRasterLayer` to WGSL. A fetch thread pulls `weather/windtex/latest.json` and the forecast-hour u/v PNG nearest to now (the same GFS feed the web timeline scrubs), and a render-graph pass after the map draws one fullscreen triangle whose fragment shader unprojects each pixel to the map plane, inverts web mercator to lng/lat, samples the equirect texture, and colormaps `length(u,v)` with the web's exact ramp (saturating at 28 m/s). `STORMDECK_WIND_OPACITY` overrides the 0.6 default (0 disables it); the ramp output is gamma-decoded before write since the wgpu surface is sRGB, unlike the web's framebuffer.

`src/wind/particles.rs` is the wgpu port of the deck-wind-layer / webgl-wind particle technique, upgraded by the platform: particle state lives in a storage buffer advected by a **compute pass** (no WebGL float-texture ping-pong), sampling the same u/v texture and moving in conformal web-mercator space (one cos(lat) scale for both axes, clamped at the poles). Points are stamped into a persistent trails texture that a fade pass decays each frame — with webgl-wind's `floor(fade × 255)/255` trick, without which u8 quantization leaves permanent ghosts — then composited over the raster. Particles spawn inside the visible viewport (density survives zoom), respawn on a speed-weighted drop rate, and trails fade harder while the camera moves since they live in screen space. Speed is deliberately exaggerated (`SPEED_FACTOR`), the same lie every wind map tells.

**Timeline**: the panel scrubs the full 7-day forecast axis (57 three-hourly steps — the same axis the web timeline drives), with a play button that loops. All hours are prefetched nearest-to-now-first (the first arrival lights the layer, the rest fill in behind) and uploaded as individual GPU textures with per-hour bind groups; the raster and particle passes pick the nearest *loaded* hour each frame, so scrubbing ahead of the prefetch shows the closest available step until the target lands. PNGs are disk-cached per snapshot under `target/wind-cache/` (relaunches skip the ~55 MB refetch; stale snapshots are pruned). Trails persist across steps, so playing the timeline shows the flow *evolving*.
